...
for (c in 1:length(C_range)){
  ...
  # Divide the whloe traing set into training and validation sets
...
## Use the best class size C=9 on testing set
```{r}
...
train_params<- EM_for_CF(movie_train, C=9, iterations=12)    
pi_mat<- train_params$pi_mat
gamma_array<- train_params$gamma_array
print(paste("C =", 9, "training Step finished!"))
...
for(i in 1:num_of_users_test){
  ...
  for(j in 1:dim(sub_test)[1]){
    ...
    S<- 0
    for(score in 1:6){
      S<- S + score * sum(pi_mat[user_index, ] * gamma_array[movies_index, score, 1:9])
    }
      est_Score<- append(est_Score, S)
  }
}
...
print(paste("C =", 9, "testing step finished!"))
```
